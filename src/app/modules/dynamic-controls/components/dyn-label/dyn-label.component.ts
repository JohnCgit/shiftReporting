...
export class DynLabelComponent implements OnInit {
  ...
  ngOnInit() {
    this.clService.getData().subscribe(options => {    
      ...
      if(this.options.control.settings) this.options.control.settings = this.options.control.settings.concat(this.initFont());
      // console.log(this.options.control);
      // console.log(this.options.control.settings);
    });
