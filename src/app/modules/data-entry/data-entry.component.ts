...
export class DataEntryComponent implements OnInit {
  ...
  getDataEntry() {
    ...
    this.getData$ = this.store.pipe(
      ...
      filter(data => {
        ...
        if (data.template.body.dashboard.length) return true;
        this.message.alertMessage('This template has no controls. You need to set up controls to use the template');
        return false;
      }),
      take(1),
      tap((cDataEntry: DataEntry) => {
        Object.assign(opt, cDataEntry);
        // console.log(opt)
        //    this.startDate = cDataEntry.createDate,
        //    this.endDate = this.parse(cDataEntry.submitDate)        // this.startDate = cDataEntry.startDate;
  ...
  submitDataEntry() {
    ...
    this.dataEntry.createDate = new Date();
